...
export default function RootPage() {
  ...
  const { role, clientId, clientName, version } = useAppSelector(state => state.demo);
    ...
    switch(role) {
      case 'merchant':
        return [
          ...
          { href: '/campaigns', label: 'Campaigns', icon: <RocketLaunchIcon className="w-5 h-5" /> },
          { href: '/analytics', label: 'Analytics', icon: <ChartBarIcon className="w-5 h-5" /> },
        ];
      case 'support':
        return [
          { href: '/dashboard', label: 'Dashboard', icon: <HomeIcon className="w-5 h-5" /> },
          { href: '/tickets', label: 'Tickets', icon: <TicketIcon className="w-5 h-5" /> },
          { href: '/merchants', label: 'Merchants', icon: <BuildingStorefrontIcon className="w-5 h-5" /> },
        ];
      case 'admin':
        return [
          { href: '/dashboard', label: 'Dashboard', icon: <HomeIcon className="w-5 h-5" /> },
          { href: '/merchants', label: 'Merchants', icon: <UserGroupIcon className="w-5 h-5" /> },
          { href: '/analytics', label: 'Analytics', icon: <ChartBarIcon className="w-5 h-5" /> },
        ];
      default:
        return [
  ...
  const getDemoLinks = () => {
    if (role === 'merchant' && clientId === 'deacons-pizza') {
      return [
        { href: '/demos/ai-campaign-creation', label: 'AI Campaign Creator', icon: '🤖' },
      ];
    }
    
    if (role === 'support' && clientId === 'cvs') {
      return [
        { href: '/demos/cvs-token-management', label: 'ExtraCare Token Management', icon: '🎟️' },
      ];
    }
        ...
        <div className="container mx-auto py-12 px-4">
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="bg-white rounded-lg shadow-md p-6">
                <div className="text-center mb-6">
                  <div className="h-24 w-24 bg-blue-100 rounded-full mx-auto flex items-center justify-center mb-4">
                    <span className="text-4xl">{clientEmojis[clientId] || '🏢'}</span>
                  </div>
                  <h2 className="text-xl font-semibold text-gray-800">{clientName || clientId}</h2>
                  <p className="text-gray-600 mt-2">
                    Welcome, {userProfile?.firstName || userProfile?.name || 'User'}
                  </p>
                ...
                <div className="space-y-4">
                  <div>
                    <div className="text-sm font-medium text-gray-600 mb-2">Current Version</div>
                    <div className="text-sm bg-gray-100 rounded-md p-2">
                      {versionInfo[version as VersionType]?.name || 'Current Release'}
                    </div>
                  </div>
                  
                  <div>
                    <div className="text-sm font-medium text-gray-600 mb-2">Mode</div>
                    <div className="text-sm bg-gray-100 rounded-md p-2">
                      {version === 'current' ? 'Production' : 
                       version === 'upcoming' ? 'Preview' : 
                       version === 'future' ? 'Prototype' : 
                       'Experimental'}
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-white rounded-lg shadow-md p-6">
                <h2 className="text-lg font-semibold mb-4 text-gray-800">Navigate to</h2>
                ...
                <div className="space-y-3">
                  ...
                  {getDemoLinks().length > 0 && (
                    <>
                      ...
                      <h3 className="text-sm font-medium text-gray-600 mb-2">Special Demos</h3>
              ...
              <div className="bg-white rounded-lg shadow-md p-6">
                <h2 className="text-lg font-semibold mb-4 text-gray-800">Demo Settings</h2>
                ...
                {/* Version Selection */}
                <div className="mb-6">
                  <h3 className="text-sm font-medium text-gray-700 mb-3">Version</h3>
