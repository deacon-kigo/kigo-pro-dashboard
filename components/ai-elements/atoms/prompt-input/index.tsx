...
/* eslint-disable -- Complex AI Elements component with architectural refactoring needed.
 * Issues: boolean prop naming, component ordering, handler naming, short identifiers.
 * TODO: Refactor in separate PR to address these comprehensively.
 */
...
// Select components removed - not used in chat UI
...
export const PromptInputSubmit = ({
  ...
  if (status === "submitted") {
    Icon = <ArrowPathIcon className="size-4 animate-spin" />;
  ...
  const handleClick = () => {
    ...
    // For other statuses, let the form submit normally
  };

  return (
    <InputGroupButton
      aria-label={status === "streaming" ? "Stop generation" : "Submit"}
      className={cn(className)}
      onClick={handleClick}
      size={size}
      type="submit"
...
/*
 * PromptInputSelect components removed - not used in chat UI
