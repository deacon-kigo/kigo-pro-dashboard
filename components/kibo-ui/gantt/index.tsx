...
export const GanttSidebarHeader: FC = () => (
  // Header strip kept for vertical alignment with the timeline axis, but the
  // original "Issues" / "Duration" labels are dropped — they're presumptuous
  // about content. Surfaces that want labels should compose them via
  // GanttSidebarGroup's `name` prop instead.
  <div
    className="sticky top-0 z-10 flex shrink-0 items-end justify-between gap-2.5 border-border/50 border-b bg-inherit p-2.5"
    style={{ height: "var(--gantt-header-height)" }}
  />
);
...
export type GanttFeatureItemCardProps = Pick<GanttFeature, "id"> & {
  children?: ReactNode;
  /** When false, the card renders without any dnd-kit wiring — useful for
   * read-only Gantt views where bars must not be picked up. Defaults to true. */
  draggable?: boolean;
};
...
export const GanttFeatureItemCard: FC<GanttFeatureItemCardProps> = ({
  ...
  useEffect(
    () => setDragging(draggable && isPressed),
    ...
    <Card className="h-full w-full rounded-md bg-background p-2 text-xs shadow-sm">
      <div
        className={cn(
          ...
          draggable && isPressed && "cursor-grabbing"
        )}
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* When `onMove` isn't provided, this is a view-only Gantt — skip
            the DndContext entirely AND tell the card to drop dnd wiring so
            bars can't be picked up, dragged, or announced as draggable to
            screen readers. */}
        {onMove ? (
          <DndContext
            modifiers={[restrictToHorizontalAxis]}
            ...
            sensors={[mouseSensor]}
          >
            <GanttFeatureItemCard id={feature.id} draggable>
              {children ?? (
