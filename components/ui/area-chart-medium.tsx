...
const DetailedUpTrendIcon: React.FC<{
  baseColor: string;
  strokeColor: string;
  className?: string;
...
const DetailedDownTrendIcon: React.FC<{
  baseColor: string;
  strokeColor: string;
  className?: string;
}> = ({ baseColor, strokeColor, className }) => (
...
const SummaryUpArrowIcon: React.FC<{ strokeColor: string }> = ({
  strokeColor,
}) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
...
const SummaryDownArrowIcon: React.FC<{ strokeColor: string }> = ({
  strokeColor,
}) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
...
const initialChartData: ChartSeries[] = [
  ...
  },
  {
    ...
    data: Array.from({ length: 7 }, (_, i) => ({
      key: generateDate(6 - i),
      data: Math.floor(Math.random() * 40) + 15,
    })),
  },
];
    ...
    <>
      <style jsx global>{`
        ...
        .dark {
          ...
          --reaviz-gridline-stroke: rgba(
            74,
            85,
            104,
            0.6
          ); /* Darker, less opaque gridline */
        }
      `}</style>
      <div className="flex flex-col justify-between pt-4 pb-4 bg-white dark:bg-black rounded-3xl shadow-[11px_21px_3px_rgba(0,0,0,0.06),14px_27px_7px_rgba(0,0,0,0.10),19px_38px_14px_rgba(0,0,0,0.13),27px_54px_27px_rgba(0,0,0,0.16),39px_78px_50px_rgba(0,0,0,0.20),55px_110px_86px_rgba(0,0,0,0.26)] w-full max-w-2xl min-h-[714px] overflow-hidden transition-colors duration-300">
        ...
        <div className="reaviz-chart-container h-[280px] px-2">
          {" "}
          {/* Adjusted height and added padding for axis labels */}
          <AreaChart
        ...
        <div className="flex flex-col sm:flex-row w-full pl-8 pr-8 justify-between pb-2 pt-8 gap-4 sm:gap-8">
          {INCIDENT_STATS_DATA.map((stat) => (
            <div key={stat.id} className="flex flex-col gap-2 w-full sm:w-1/2">
              <span className="text-xl text-gray-800 dark:text-gray-200 transition-colors duration-300">
                {stat.title}
              </span>
              <div className="flex items-center gap-2">
                ...
                />
                <div
                  className={`flex ${stat.trendBgColor} p-1 pl-2 pr-2 items-center rounded-full ${stat.trendColor}`}
                >
                  <stat.TrendIconSvg
                    strokeColor={
                      stat.trendColor.startsWith("text-[#F08083]")
                        ? "#F08083"
                        : "#40E5D1"
                    }
                  />
          ...
          {DETAILED_METRICS_DATA.map((metric) => (
            ...
            >
              <div className="flex flex-row gap-2 items-center text-base w-1/2 text-gray-500 dark:text-gray-400 transition-colors duration-300">
                <metric.Icon
                  fill={
                    metric.id === "ier" && metric.trendBaseColor === "#40E5D1"
                      ? "#40E5D1"
                      : "#E84045"
                  }
                />
              ...
              <div className="flex gap-2 w-1/2 justify-end items-center">
                <span className="font-semibold text-xl text-gray-900 dark:text-white transition-colors duration-300">
                  {metric.value}
                </span>
                <metric.TrendIcon
