...
interface CombinedOfferFormP0MerchantProps {
  formData: any;
...
export default function CombinedOfferFormP0Merchant({
  formData,
  onUpdate,
}: CombinedOfferFormP0MerchantProps) {
  ...
  // Handle merchant selection from hybrid search
  const handleMerchantSelect = (merchant: MerchantData) => {
    setSelectedMerchant(merchant);
    onUpdate("merchantData", merchant);

    // If it's an existing merchant, also set the merchant ID
    if (merchant.source === "existing" && merchant.id) {
      onUpdate("merchant", merchant.id);
    } else {
      onUpdate("merchant", "");
    }
  };
  ...
  // Handle clearing merchant selection
  const handleClearSelection = () => {
    setSelectedMerchant(null);
    onUpdate("merchantData", null);
    onUpdate("merchant", "");
  };
  ...
  const handleManualEntrySave = (merchantData: MerchantData) => {
    ...
    onUpdate("merchantData", merchantData);
    onUpdate("merchant", "");
  ...
  return (
    <div className="space-y-6">
      {/* Merchant Selection Section - Hybrid Search */}
