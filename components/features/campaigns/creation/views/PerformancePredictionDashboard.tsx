...
> = ({ predictions, campaignParams, onUpdateParams }) => {
  ...
  const [addPushNotification, setAddPushNotification] = useState(false);
  const [appliedSuggestions, setAppliedSuggestions] = useState<string[]>([]);
  const [debugMode] = useState(true); // Enable for console logging

  // Debug effect to monitor state changes
  React.useEffect(() => {
    if (debugMode) {
      console.log("Applied suggestions updated:", appliedSuggestions);
    }
  }, [appliedSuggestions, debugMode]);
    ...
    switch (key) {
      case "views":
        ...
        updatedPredictions.redemptions = {
          min: Math.round(
            predictions.redemptions.min * (1 + (percentage / 100) * 0.8)
          ),
          max: Math.round(
            predictions.redemptions.max * (1 + (percentage / 100) * 0.8)
          ),
        };
        updatedPredictions.revenue = {
          min: Math.round(
            predictions.revenue.min * (1 + (percentage / 100) * 0.8)
          ),
          max: Math.round(
            predictions.revenue.max * (1 + (percentage / 100) * 0.8)
          ),
        };
        break;
      case "radius":
        ...
        updatedPredictions.redemptions = {
          min: Math.round(
            predictions.redemptions.min * (1 + (percentage / 100) * 0.7)
          ),
          max: Math.round(
            predictions.redemptions.max * (1 + (percentage / 100) * 0.7)
          ),
        };
        updatedPredictions.revenue = {
          min: Math.round(
            predictions.revenue.min * (1 + (percentage / 100) * 0.7)
          ),
          max: Math.round(
            predictions.revenue.max * (1 + (percentage / 100) * 0.7)
          ),
        };
        break;
    }
  ...
  // Handle scenario toggle with fixes for the add buttons
  const handleScenarioToggle = (suggestionId: string, enabled: boolean) => {
    if (debugMode) {
      console.log(`Toggling suggestion ${suggestionId} to ${enabled}`);
      console.log("Current applied suggestions:", appliedSuggestions);
    }

    // Create a completely new array for React to detect the change
    let newAppliedSuggestions: string[];

    if (enabled) {
      newAppliedSuggestions = [...appliedSuggestions, suggestionId];
    } else {
      newAppliedSuggestions = appliedSuggestions.filter(
        (id) => id !== suggestionId
      );
    }
    ...
    if (debugMode) {
      console.log("New applied suggestions:", newAppliedSuggestions);
    }

    // Apply specific changes based on suggestion type
    switch (suggestionId) {
      case "suggestion-1": // Extend to Weekend
        setIncludeFriday(enabled);

        if (enabled) {
          // Add days to days array if not already included
          const newDays = [...campaignParams.targeting.days];
          ["friday", "saturday", "sunday"].forEach((day) => {
            if (!newDays.includes(day)) {
              newDays.push(day);
            }
          });
          onUpdateParams("targeting.days", newDays);
        } else {
          // Remove weekend days from days array
          onUpdateParams(
            "targeting.days",
            campaignParams.targeting.days.filter(
              (day) => !["friday", "saturday", "sunday"].includes(day)
            )
          );
        }
        break;

      case "suggestion-2": // Add Push Notification
        setAddPushNotification(enabled);
        // Simulate adding push notification by adjusting metrics
        // Would update campaign parameters in a real implementation
        break;

      case "suggestion-3": // Increase Radius
        setIncreasedRadius(enabled);

        if (enabled) {
          // Increase radius by 1 mile
          onUpdateParams(
            "targeting.radius",
            campaignParams.targeting.radius + 1
          );
        } else {
          // Decrease radius by 1 mile
          onUpdateParams(
            "targeting.radius",
            Math.max(1, campaignParams.targeting.radius - 1)
          );
        }
        break;
    }
      ...
      <div className="p-6">
        ...
        >
          ...
          <p className="text-gray-600 text-sm">
            Based on your campaign parameters, here's what you can expect. Try
            different scenarios to optimize your results.
          </p>
        </motion.div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100 mb-6">
          ...
          <div className="mt-2 bg-gray-50 p-3 rounded-lg">
            <p className="text-sm text-gray-700">
              <span className="font-semibold">Key Insight:</span> This campaign
              is predicted to increase your monthly revenue by up to{" "}
              {Math.round(
                (predictions.revenue.max / predictions.revenue.min - 1) * 100
              )}
              %.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
            ...
            <div className="mt-2 bg-gray-50 p-3 rounded-lg">
              <p className="text-sm text-gray-700">
                <span className="font-semibold">Key Insight:</span> About{" "}
                {Math.round(
                  (predictions.customerFlow[2].count /
                    predictions.customerFlow[0].count) *
                    100
                )}
                % of users who view your offer are expected to redeem it.
              </p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
            ...
            <div className="mt-2 bg-gray-50 p-3 rounded-lg">
              <p className="text-sm text-gray-700">
                ...
                campaign is expected to outperform industry averages in
                conversion rate by{" "}
                {Math.round(
                  predictions.competitiveComparison[1].value -
                    predictions.competitiveComparison[1].benchmark
                )}
                %.
              </p>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {predictions.suggestions.map((suggestion) => {
              // Explicitly check if the suggestion is active to ensure reliable rendering
              const isSuggestionActive = appliedSuggestions.includes(
                suggestion.id
              );

              return (
                <OptimizationCard
                  key={`suggestion-${suggestion.id}-${isSuggestionActive ? "active" : "inactive"}`}
                  suggestion={suggestion}
                  onApply={() => {
                    // Apply the suggestion logic using the suggestion ID directly
                    handleScenarioToggle(suggestion.id, !isSuggestionActive);
                  }}
...
const OptimizationCard: React.FC<{
  ...
  // Keep local state to ensure stability during transitions
  ...
  // Using separate class variables instead of string interpolation to avoid purging issues
  const cardClass = localIsActive
    ? "rounded-lg border border-green-500 bg-green-50 p-3"
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    // Then call the parent handler after a small delay
    setTimeout(() => {
      onApply();
    }, 10);
  };
  ...
  return (
    <div className={cardClass}>
      ...
      <div className="mt-3 flex justify-end">
        <Button
          size="xs"
          variant={localIsActive ? "primary" : "outline"}
          onClick={handleClick}
        >
          {localIsActive ? "Applied" : "Apply"}
        </Button>
      </div>
    </div>
  );
