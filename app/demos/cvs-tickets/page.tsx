...
import {
  TicketStatusBadge,
  TierBadge,
} from "@/components/molecules/badges/TicketBadge";
...
export default function CVSTicketsView() {
  ...
  const { tickets, selectedTicket: reduxSelectedTicket } = useAppSelector(
    (state) => state.cvsToken
  );
  ...
  const [currentFilter, setCurrentFilter] = useState<
    "all" | "tier1" | "tier2" | "escalated" | "closed"
  >("all");
  ...
  const externalSystemName =
    features?.ticketing?.externalSystemName || "External System";
  ...
  const TicketDetailsPanel = () => {
    if (!selectedTicket) {
      return (
        <div
          className="bg-white rounded-lg border border-gray-100 p-8 text-center h-full flex flex-col items-center justify-center"
    ...
    return (
      ...
      >
        {/* Ticket Header */}
        <div
          className={`p-4 border-b border-gray-100 ${selectedTicket.tier === "Tier1" ? "bg-blue-50" : "bg-red-50"}`}
        >
          ...
          <div className="mt-2 flex flex-wrap items-center text-sm text-gray-600 gap-2">
            <span
              className={`px-2 py-0.5 rounded-full border ${PRIORITY_COLORS[selectedTicket.priority]}`}
            >
            ...
            selectedTicket.status !== "Escalated" && (
              <div className="p-4 border-b border-gray-100 bg-yellow-50">
                ...
                <p className="text-xs text-yellow-700 mb-2">
                  Escalate this ticket to Kigo PRO specialists for advanced
                  token management assistance.
                </p>
                <textarea
                        ...
                        <div className="flex items-start space-x-2">
                          ...
                          <div>
                            <div className="flex items-center">
                              ...
                              <span
                                className={`ml-2 px-1.5 py-0.5 text-xs rounded-full ${PRIORITY_COLORS[ticket.priority]}`}
                              >
                                {ticket.priority}
                              </span>
                            </div>
