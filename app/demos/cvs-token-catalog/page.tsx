...
export default function CVSTokenCatalog() {
  ...
  useEffect(() => {
    ...
    if (themeMode !== "light") {
      router.replace(
        "/demos/cvs-token-catalog?role=support&client=cvs&scenario=token-management&theme=light"
      );
    }
  }, [themeMode, router]);
  ...
  const generateTokenCatalog = (): TokenInfo[] => {
    ...
    for (let i = 1; i <= 30; i++) {
      ...
      let value: string;
      if (type === "Coupon") {
        const discountPercent = [10, 15, 20, 25, 30, 40, 50][
  ...
  const generateTokenDescription = (
    ...
    if (type === "Coupon")
      return `Get ${value} off your purchase of ${category.toLowerCase()}.`;
    if (type === "ExtraBucks")
      return `Earn ${value} ExtraBucks Rewards when you spend $${parseInt(value.replace("$", "")) * 4} on ${category.toLowerCase()}.`;
    if (type === "Reward")
      return `${value} reward for your loyalty. Redeem on your next purchase of ${category.toLowerCase()}.`;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
                ...
                {filteredTokens.map((token) => (
                  ...
                  >
                    <div className="flex justify-between items-start">
                      ...
                      <span
                        className={`px-2 py-1 text-xs font-semibold rounded-full ${getTokenTypeBadgeColor(token.type)}`}
                      >
                        {token.type}
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between items-center mt-auto">
                      ...
                      <div className="flex space-x-2">
                        ...
                        <button
                          ...
                          onClick={() =>
                            router.push(
                              `${buildDemoUrl("cvs", "token-management")}?addToken=${token.id}`
                            )
                          }
                        >
      ...
      <Link
        ...
        className="absolute bottom-20 right-8 bg-blue-600 text-white p-3 rounded-full shadow-lg hover:bg-blue-700 transition-colors z-10"
